...
describe('ASK_PROMPT', () => {
    ...
    it('should maintain consistent ask prompt', async () => {
        const prompt = await ASK_PROMPT('/test/path', false, // supportsComputerUse
        undefined, // mcpHub
    ...
    it('should include browser actions when supportsComputerUse is true', async () => {
        const prompt = await ASK_PROMPT('/test/path', true, undefined, undefined, '1280x800');
        expect(prompt).toMatchSnapshot();
    });
    it('should include MCP server info when mcpHub is provided', async () => {
        mockMcpHub = createMockMcpHub();
        const prompt = await ASK_PROMPT('/test/path', false, mockMcpHub);
        expect(prompt).toMatchSnapshot();
    });
    it('should explicitly handle undefined mcpHub', async () => {
        const prompt = await ASK_PROMPT('/test/path', false, undefined, // explicitly undefined mcpHub
        undefined, undefined);
        expect(prompt).toMatchSnapshot();
    });
    it('should handle different browser viewport sizes', async () => {
        const prompt = await ASK_PROMPT('/test/path', true, undefined, undefined, '900x600' // different viewport size
        );
        expect(prompt).toMatchSnapshot();
    });
    it('should include diff strategy tool description', async () => {
        const prompt = await ASK_PROMPT('/test/path', false, undefined, new SearchReplaceDiffStrategy(), // Use actual diff strategy from the codebase
        undefined);
        expect(prompt).toMatchSnapshot();
    });
    afterAll(() => {
