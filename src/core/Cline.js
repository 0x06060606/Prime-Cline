...
export class Cline {
    ...
    constructor(provider, apiConfiguration, customInstructions, enableDiff, fuzzyMatchThreshold, task, images, historyItem) {
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const systemPrompt = await SYSTEM_PROMPT(cwd, this.api.getModel().info.supportsComputerUse ?? false, mcpHub, this.diffStrategy, browserViewportSize, mode) + await addCustomInstructions(this.customInstructions ?? '', cwd, preferredLanguage);
                    ...
                    case "apply_diff": {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                const diffResult = this.diffStrategy?.applyDiff(originalContent, diffContent, parseInt(block.params.start_line ?? ''), parseInt(block.params.end_line ?? '')) ?? {
                                    success: false,
