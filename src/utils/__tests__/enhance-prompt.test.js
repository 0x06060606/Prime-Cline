...
describe('enhancePrompt', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        buildApiHandler.mockReturnValue(mockHandler);
    });
    it('should throw error for non-OpenRouter providers', async () => {
        const nonOpenRouterConfig = {
    ...
    it('should enhance a valid prompt', async () => {
        ...
        expect(mockHandler.completePrompt).toHaveBeenCalledWith(expect.stringContaining(inputPrompt));
    });
    ...
    it('should pass the correct prompt format to the API', async () => {
        ...
        expect(mockHandler.completePrompt).toHaveBeenCalledWith('Generate an enhanced version of this prompt (reply with only the enhanced prompt - no conversation, explanations, lead-in, bullet points, placeholders, or surrounding quotes):\n\nTest prompt');
    });
});
