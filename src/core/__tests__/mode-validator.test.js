...
describe('mode-validator', () => {
    describe('isToolAllowedForMode', () => {
        ...
        describe('architect mode', () => {
            it('allows only read-only and MCP tools', () => {
                ...
                const disallowedTools = ['execute_command', 'write_to_file', 'apply_diff'];
                disallowedTools.forEach(tool => {
                    expect(isToolAllowedForMode(tool, architectMode)).toBe(false);
                });
            });
        });
        describe('ask mode', () => {
            it('allows only read-only and MCP tools', () => {
                ...
                const disallowedTools = ['execute_command', 'write_to_file', 'apply_diff'];
                disallowedTools.forEach(tool => {
                    expect(isToolAllowedForMode(tool, askMode)).toBe(false);
                });
            });
        });
    });
    describe('validateToolUse', () => {
        it('throws error for disallowed tools in architect mode', () => {
            expect(() => validateToolUse('write_to_file', architectMode)).toThrow('Tool "write_to_file" is not allowed in architect mode.');
        });
        it('throws error for disallowed tools in ask mode', () => {
            expect(() => validateToolUse('execute_command', askMode)).toThrow('Tool "execute_command" is not allowed in ask mode.');
        });
        it('throws error for unknown tools in code mode', () => {
            expect(() => validateToolUse(asTestTool('unknown_tool'), codeMode)).toThrow('Tool "unknown_tool" is not allowed in code mode.');
        });
        it('does not throw for allowed tools', () => {
            // Code mode
            expect(() => validateToolUse('write_to_file', codeMode)).not.toThrow();
            // Architect mode
            expect(() => validateToolUse('read_file', architectMode)).not.toThrow();
            // Ask mode
            expect(() => validateToolUse('browser_action', askMode)).not.toThrow();
        });
    });
});
