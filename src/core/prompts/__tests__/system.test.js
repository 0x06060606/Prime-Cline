...
describe('SYSTEM_PROMPT', () => {
    ...
    it('should maintain consistent system prompt', async () => {
        const prompt = await SYSTEM_PROMPT('/test/path', false, // supportsComputerUse
        undefined, // mcpHub
    ...
    it('should explicitly handle undefined mcpHub', async () => {
        const prompt = await SYSTEM_PROMPT('/test/path', false, undefined, // explicitly undefined mcpHub
        undefined, undefined);
        expect(prompt).toMatchSnapshot();
    });
    it('should handle different browser viewport sizes', async () => {
        const prompt = await SYSTEM_PROMPT('/test/path', true, undefined, undefined, '900x600' // different viewport size
        );
        expect(prompt).toMatchSnapshot();
    });
    it('should include diff strategy tool description', async () => {
        const prompt = await SYSTEM_PROMPT('/test/path', false, undefined, new SearchReplaceDiffStrategy(), // Use actual diff strategy from the codebase
        undefined);
        expect(prompt).toMatchSnapshot();
    });
    afterAll(() => {
...
describe('addCustomInstructions', () => {
    ...
    it('should include preferred language when provided', async () => {
        const result = await addCustomInstructions('', '/test/path', 'Spanish');
        expect(result).toMatchSnapshot();
    });
    it('should include custom instructions when provided', async () => {
        const result = await addCustomInstructions('Custom test instructions', '/test/path');
        expect(result).toMatchSnapshot();
    });
    it('should include rules from .clinerules', async () => {
        const result = await addCustomInstructions('', '/test/path');
        expect(result).toMatchSnapshot();
    });
    it('should combine all custom instructions', async () => {
        const result = await addCustomInstructions('Custom test instructions', '/test/path', 'French');
        expect(result).toMatchSnapshot();
    });
    afterAll(() => {
        jest.restoreAllMocks();
