...
declare module "vscode" {
    ...
    // Remove or comment out the conflicting type declarations
    // namespace LanguageModelChatMessage {
    //     function User(text: string): LanguageModelChatMessage
    //     function Assistant(text: string): LanguageModelChatMessage
    // }
    // https://github.com/microsoft/vscode/blob/a370576a871658dcb36b4d112636fa2501fda313/src/vscode-dts/vscode.d.ts#L19413
    interface LanguageModelChat {
        sendRequest(messages: LanguageModelChatMessage[], options: any, token: any): Promise<any>
    }
...
export class GitHubCopilotNativeHandler implements ApiHandler {
    ...
    async *createMessage(systemPrompt: string, messages: MessageParam[]): ApiStream {
        ...
        const lmChatMessages = [
            { 
                role: 'user' as unknown as vscode.LanguageModelChatMessageRole,
            ...
            if (typeof (message.content) === 'string') {
                ...
                if (message.role === 'user') {
                    ...
                    for (const content of message.content) {
                        if (content.type === 'text') {
                            lmChatMessages.push({ 
                                role: 'assistant' as unknown as vscode.LanguageModelChatMessageRole, 
                                content: [{ type: 'text', value: content.text }],
                                name: undefined
                            })
                        }
