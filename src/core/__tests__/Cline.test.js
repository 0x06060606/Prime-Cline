    ...
    describe('getEnvironmentDetails', () => {
        ...
        describe('API conversation handling', () => {
            it('should clean conversation history before sending to API', async () => {
                ...
                const relevantCall = calls.find(call => call[1]?.some((msg) => msg.content?.[0]?.text === 'test message'));
                // Verify the conversation history was cleaned in the relevant call
                expect(relevantCall?.[1]).toEqual(expect.arrayContaining([
                    {
                        role: 'user',
                        content: [{ type: 'text', text: 'test message' }]
                    }
                ...
                const passedMessage = relevantCall?.[1].find((msg) => msg.content?.[0]?.text === 'test message');
            ...
            describe('loadContext', () => {
                it('should process mentions in task and feedback tags', async () => {
                    ...
                    expect(processedContent[0].text)
                        .toBe('Regular text with @/some/path');
                    // Text within task tags should be processed
                    expect(processedContent[1].text)
                        .toContain('processed:');
