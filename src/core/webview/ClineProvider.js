...
export class ClineProvider {
    ...
    workspaceTracker;
    mcpHub;
    latestAnnouncementId = "dec-10-2024"; // update to some unique identifier when we add a new announcement
    configManager;
    ...
    */
    ...
    async initClineWithTask(task, images) {
        await this.clearTask();
        const { apiConfiguration, customInstructions, diffEnabled, fuzzyMatchThreshold } = await this.getState();
        this.cline = new Cline(this, apiConfiguration, customInstructions, diffEnabled, fuzzyMatchThreshold, task, images);
    }
    async initClineWithHistoryItem(historyItem) {
        await this.clearTask();
        const { apiConfiguration, customInstructions, diffEnabled, fuzzyMatchThreshold } = await this.getState();
        this.cline = new Cline(this, apiConfiguration, customInstructions, diffEnabled, fuzzyMatchThreshold, undefined, undefined, historyItem);
    }
    ...
    async getStateToPostToWebview() {
        const { apiConfiguration, lastShownAnnouncementId, customInstructions, alwaysAllowReadOnly, alwaysAllowWrite, alwaysAllowExecute, alwaysAllowBrowser, alwaysAllowMcp, soundEnabled, diffEnabled, taskHistory, soundVolume, browserViewportSize, screenshotQuality, preferredLanguage, writeDelayMs, terminalOutputLineLimit, fuzzyMatchThreshold, mcpEnabled, alwaysApproveResubmit, requestDelaySeconds, currentApiConfigName, listApiConfigMeta, mode, } = await this.getState();
    ...
    */
    async getState() {
        const [storedApiProvider, apiModelId, apiKey, glamaApiKey, glamaModelId, glamaModelInfo, openRouterApiKey, awsAccessKey, awsSecretKey, awsSessionToken, awsRegion, awsUseCrossRegionInference, vertexProjectId, vertexRegion, openAiBaseUrl, openAiApiKey, openAiModelId, ollamaModelId, ollamaBaseUrl, lmStudioModelId, lmStudioBaseUrl, anthropicBaseUrl, geminiApiKey, openAiNativeApiKey, deepSeekApiKey, azureApiVersion, openAiStreamingEnabled, openRouterModelId, openRouterModelInfo, openRouterUseMiddleOutTransform, lastShownAnnouncementId, customInstructions, alwaysAllowReadOnly, alwaysAllowWrite, alwaysAllowExecute, alwaysAllowBrowser, alwaysAllowMcp, taskHistory, allowedCommands, soundEnabled, diffEnabled, soundVolume, browserViewportSize, fuzzyMatchThreshold, preferredLanguage, writeDelayMs, screenshotQuality, terminalOutputLineLimit, mcpEnabled, alwaysApproveResubmit, requestDelaySeconds, currentApiConfigName, listApiConfigMeta, mode, modeApiConfigs,] = await Promise.all([
